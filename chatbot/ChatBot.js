...
import { Dimensions, TextInput, ScrollView, Platform, Keyboard , View } from 'react-native';
...
import { MasksInput } from '../UI/form/input/masksInput';
...
class ChatBot extends Component {
    ...
    if (isEnd) {
      ...
      if (nextStep.user) {
        if (nextStep.type == 'password') {
          this.setState({ ...INITIAL_INPUT,securityTextEntry:true, returnKeyType:'next', keyboardType:'default',placeholder:'********', editable:true })
        }else if (nextStep.type == 'email') {
          this.setState({ ...INITIAL_INPUT,placeholder:'nome@mail.com', returnKeyType:'next',keyboardType:'email-address' , editable:true })
        }else if (nextStep.type == 'sms') {
          this.setState({ ...INITIAL_INPUT,placeholder:'0000', returnKeyType:'next',keyboardType:'numeric' , maxLength:4, editable:true })
        }else if (nextStep.type == 'code') {
          this.setState({ ...INITIAL_INPUT,placeholder:'código', returnKeyType:'next', keyboardType:'default' , maxLength:30, editable:true })
        }else if (nextStep.type == 'mask') {
          this.setState({ ...INITIAL_INPUT,
          ...
          this.setState({ ...INITIAL_INPUT,placeholder:'Mensagem', editable:true })
        }
  ...
  checkInvalidInput() {
    const { currentStep, inputValue, securityTextEntry } = this.state;
    ...
    if (typeof result !== 'boolean' || !result) {
      ...
      setTimeout(() => {
        if (currentStep.type=='password') {
          this.setState({securityTextEntry:true});
        }
        if (currentStep.type=='mask') {
          this.setState({
            options:{
              mask: '(99) 99999-9999'
            },
  ...
  render() {
    ...
    const textMaskStyle = Object.assign({}, styles.mask, inputStyle);
    ...
    if (currentStep.type=='mask') {
      return (
        ...
        >
          ...
          <InputView behavior={platformBehavior} keyboardVerticalOffset={keyboardVerticalOffset} enabled>
            <Footer
            ...
            >
              <MasksInput
                ...
                type={'custom'}
                inputStyle={[textMaskStyle]}
                className="rsc-input"
                placeholder={this.state.placeholder}
                returnKeyType={this.state.returnKeyType}
                secureTextEntry={this.state.securityTextEntry}
                keyboardType={this.state.keyboardType}
                reference={ref => this.maskInputRef = ref}
...
ChatBot.propTypes = {
  ...
  botAvatar: PropTypes.any,
  botBubbleColor: PropTypes.string,
  botDelay: PropTypes.number,
  botFontColor: PropTypes.string,
  botFontWeight: PropTypes.string,
  botFontSize: PropTypes.number,
